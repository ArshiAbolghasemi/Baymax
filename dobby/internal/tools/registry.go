...
// Set owns the resources the tools share: one HTTP client for the upstream
// sources, and the TTL cache in front of them.
//
...
// New builds the shared client and cache.
func New() (*Set, error) {
	...
	logging.Logger.Info("source client ready",
		...
		zap.Int("max_cache_entries", config.Conf.MaxCacheEntries),
	)

	return &Set{client: client, cache: NewCache(config.Conf.MaxCacheEntries)}, nil
}
...
func (s *Set) Close() {
	logging.Logger.Info("closing the tool resources", zap.Int("cached_entries", s.cache.Len()))
	s.cache.Clear()
