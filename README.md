...
```
eve, bashmax ──HTTP──> hiro ──MCP──> dobby ──HTTPS──> MedlinePlus, DailyMed, openFDA
                        │
                        ├─> Phoenix (prompts)
...
# ...or in a browser, with the tool calls shown:
cd ../eve && npm install && npm run dev            # http://localhost:5173
```
